...
@section('content')
    <div class="d-sm-flex text-center justify-content-between align-items-center mb-4">
        <h3 class="mb-sm-0 mb-1 fs-18">Service Details</h3>
                        ...
                        <div class="card-body p-4">
                            <div class="mb-4">
                                <span
                                    ...
                                    <form action="{{ route('Service-details.destroy', $item) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                ...
                                <span
                                    ...
                                    <a href="{{ route('Service-details.edit', $item->id) }}">
                                        Rename
                                    </a>
                                </span>
                            </div>
                            <img src="{{ asset('storage/' . $item->image) }}" class="mb-4 rounded-10" alt="{{ $item->image}}">
                            <h3 class="mb-3 fs-16 fw-semibold">{{$item->title}}</h3>
                            <p class="mb-20">{{$item->slug}}</p>
                            <a href="{{ route('Service-details.show', $item) }}"
