...
@section('content')
    <form class="row g-3 needs-validation" action="{{ route('DesignCategory.update', $designCategory->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="col-md-6">
            <br><br>
            ...
            <div class="position-relative">
                <input type="text" class="form-control h-58 @error('name') is-invalid @enderror" name="name"
                  id="validationCustom10" value="{{ old('name', $designCategory->name) }}" required>
